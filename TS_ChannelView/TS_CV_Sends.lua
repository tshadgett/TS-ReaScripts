...
--[[
  ...
  a send, seen from the other end: category 0 on the sending track,
  category -1 on the receiving one, with the same volume, mute, mode and
  channel fields. SD.make(kind) builds a panel for either side; the
  ...
  on the track you were working on.
...
-- What differs between the two sides. `cat` is REAPER's send category as
-- seen from the panel's own track; `other` is the field naming the track
-- at the far end.
local KINDS = {
  send = {
    cat = 0, other = "P_DESTTRACK", key = "##sends", id = "sd",
    title = "Sends", noun = "send", collapsed_default = false,
  },
  receive = {
    cat = -1, other = "P_SRCTRACK", key = "##receives", id = "rv",
    title = "Receives", noun = "receive", collapsed_default = true,
  },
}
...
local function is_separator(name)
...
local function widen(tr)
  if tr and (reaper.GetMediaTrackInfo_Value(tr, "I_NCHAN") or 2) < 4 then
...
local function make(kind)
  ...
  -- Receives. What a sidechain has to widen.
  local function receiver(track, i)
    if RECV then return track end
  ...
  local function track_items(ctx, track, sidechain)
    ...
    for _, t in ipairs(project_tracks(track)) do
      if t.sep then
        ...
        local label = ("   %d  %s##%s%d"):format(t.num, t.name, ID, t.num)
        -- The panel's own track is listed but disabled rather than dropped.
        -- A number missing from the middle of the list reads as a bug in the
        -- list, not as "a track can't send to itself".
  ...
  function SD.draw(ctx, track, avail_h)
    local collapsed = SD.is_collapsed()
    ...
    local ok = ImGui.BeginChild(ctx, ID .. "panel", w, avail_h, 0,
    ...
    if ok then
      ...
      if collapsed then
        ...
        if W.icon_button(ctx, ID .. "exp", "expand", btn, false,
        ...
        -- Receives sits right beside it and would only repeat it. The
        -- lamps report what the panels cannot: the parent send, and
        -- whether anything is arriving from elsewhere.
        if not RECV then
          local leds = SD.route_leds(track)
          ImGui.SetCursorScreenPos(ctx, x + ww - btn * 2 - 9, y + 3)
          if W.route_button(ctx, ID .. "route", btn, btn, leds,
              ("Routing and I/O\n%s\n%s\n%s")
        ...
        if W.icon_button(ctx, ID .. "col", "collapse", btn, false,
            "Collapse the " .. K.title:lower()) then
          toggle()
        end
  ...
  function SD.draw_sends(ctx, dl, x, y, w, h, track, sends)
    ...
    local pressed = ImGui.InvisibleButton(ctx, ID .. "add", aw, ah)
